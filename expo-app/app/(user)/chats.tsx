...
export default function UserChats() {
  ...
  const getAdvisorImage = (advisor: Chat['advisor']) => {
      ...
      const { data, error: fetchError } = await supabase
        ...
        .select(`
          *,
          ...
          advisor:profiles!chats_advisor_id_fkey(
            full_name,
            image_url,
            gender
          )
      ...
      {loading ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtext}>
            Start a conversation with an advisor from the home screen
          </Text>
        </View>
      ) : (
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Image
                source={{ uri: getAdvisorImage(item.advisor) }}
                style={styles.advisorImage}
              />
              <View style={styles.chatContent}>
                <View style={styles.chatHeader}>
                  <Text style={styles.advisorName}>
                    {item.advisor.full_name || `Advisor ${item.advisor_id.slice(0, 8)}`}
                  </Text>
                  {item.last_message && (
